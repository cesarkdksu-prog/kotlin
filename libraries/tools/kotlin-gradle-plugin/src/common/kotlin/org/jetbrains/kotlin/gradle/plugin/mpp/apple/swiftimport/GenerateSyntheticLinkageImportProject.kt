...
internal abstract class GenerateSyntheticLinkageImportProject : DefaultTask(), UsesBuildFusService {
    ...
    @get:Internal
    val coordinationDisabled: Property<Boolean> = project.objects.property(Boolean::class.java).convention(true)

    @get:InputFile
    @get:Optional
    @get:PathSensitive(PathSensitivity.NONE)
    abstract val generateSyntheticPackageHashFile: RegularFileProperty
    ...
    fun generateSwiftPMSyntheticImportProjectAndFetchPackages() {
        ...
        if (coordinationDisabled.get() || !generateSyntheticPackageHashFile.isPresent) {
            ...
            val claim = coordinationService.get().claimOrJoinPackageGeneration(
                packageHash = generateSyntheticPackageHashFile.get().asFile.readText().trim()
            )

            when (claim) {
                is SwiftPMXcodeDumpBuildService.GeneratePackageClaim.Existing -> {
                    coordinationService.get().awaitPackageGeneration(claim.bucket)
                }

                is SwiftPMXcodeDumpBuildService.GeneratePackageClaim.Owner -> runOwnerPackageGeneration(
                    claim.bucket
                )
            }
            syncFromOwner(
    ...
    private fun runOwnerPackageGeneration(
        bucket: SwiftPMXcodeDumpBuildService.GeneratePackageBucket,
    ) {
        try {
            submitPackageGenerationWorkAction(
