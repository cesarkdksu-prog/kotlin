...
class GenerateSyntheticLinkageImportProjectTests : KGPBaseTest() {
    ...
    @GradleTest
    fun `generate task generates identical packages given the same target fingerprint hash`(version: GradleVersion) {
        ...
        project("empty", version) {
            withLockFileFixture {
                ...
                build(
                    ...
                    val subProjectGeneratePackageHash = subProject.projectPath.resolve("build/kotlin/syntheticPackageHash")
                        .readText()
                        .trim()

                    val rootProjectGeneratePackageHash = projectPath.resolve("build/kotlin/syntheticPackageHash")
                        .readText()
                        .trim()
