...
**Referential-identity (`allDistinctByIdentity`) — no demand.** `IdentityHashMap` and `identityHashCode` are used widely (across ~220 files), but almost entirely as identity-based `hashCode()` implementations, identity caches, and weak maps. One identity-based duplicate guard does appear — `intellij-community/platform/extensions/.../ExtensionPointImpl.kt:377,402`, an `IdentityHashMap`-backed `MutableSet` tested with `!duplicates.add(extension)` — but it is an incremental construction-time guard (the identity analog of the §3.3 `Set.add` guards), not a finished-collection "no two referentially-equal elements" check. No whole-collection identity-distinctness shape surfaced, so deferring the `===` variant to its own ticket — as KT-85976 proposes — is well supported.
...
- **Nullable elements.** KT-85976 treats `null` as a valid `HashSet` element, so two `null` values (or two `null` selector results) count as duplicates. No direct boolean distinctness check involving `null` surfaced in the corpus. `distinctBy` over nullable elements and with `null`-producing selectors does appear — IntelliJ's jvm-debugger test data has `sequenceOf(null, 1, …).distinctBy { it == null }` (nullable elements; the selector `{ it == null }` returns `Boolean` and does not itself produce `null`) and `distinctBy { if (…) null else it }` (a selector that genuinely produces `null`) — but only as `.distinctBy {}.count()` fixtures, which §2's triage rule discards. The nullable behavior therefore rests on the KT-85976 spec, not on field confirmation.
...
| Referential `===` variant deferred | Confirmed — no whole-collection identity-distinctness check; an identity duplicate guard appears only as an incremental §3.3-style guard |
...
Ship `allDistinct()` and `allDistinctBy {}` as proposed in KT-85976. The corpus shows strong, broad, production-weighted demand; confirms the selector form and the short-circuit value; and independently justifies excluding `allDistinctWith` and deferring the referential-identity variant to its own ticket.
