...
class DebugMetadataTest {
    ...
    fun testRuntimeDebugMetadata() {
        ...
        assertEquals(["L$1", "a", "L$2", "b"], myContinuation.getSpilledVariableFieldMapping()!!.toList())
        myContinuation.label = 2
        assertEquals(
            StackTraceElement("SomeClass", "testMethod", "test.kt", 122),
            myContinuation.getStackTraceElement()
        )
        assertEquals(["L$1", "b", "L$2", "c"], myContinuation.getSpilledVariableFieldMapping()!!.toList())
        myContinuation.label = 3
        assertEquals(
            StackTraceElement("SomeClass", "testMethod", "test.kt", 11),
            myContinuation.getStackTraceElement()
        )
        assertEquals(["L$1", "c"], myContinuation.getSpilledVariableFieldMapping()!!.toList())
    }
}
