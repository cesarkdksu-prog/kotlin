...
internal abstract class IdSignatureAwareSerializer<T : FileAwareSerializedData> {
    ...
    protected open fun addStringsTo(item: T, builder: StringTableBuilder) = with(builder) {
        val file = item.file
        +file.fqName
        +file.path
    }
    ...
    protected open fun writeExtraPayload(stream: ByteArrayStream, stringTable: StringTable, item: T) = Unit
    ...
    fun serialize(items: List<T>): ByteArray {
        ...
        val stringTable = buildStringTable {
            items.forEach { addStringsTo(it, this) }
        }
        ...
        items.forEach { item ->
            ...
            writeExtraPayload(stream, stringTable, item)
        }
    ...
    fun deserializeTo(data: ByteArray, result: MutableList<T>) {
        ...
        while (stream.hasData()) {
            ...
            result.add(readItem(stream, stringTable, file, signature))
        }
    }
}
...
internal object ClassFieldsSerializer : IdSignatureAwareSerializer<SerializedClassFields>() {
    ...
    override fun addStringsTo(item: SerializedClassFields, builder: StringTableBuilder) {
        super.addStringsTo(item, builder)
    ...
    override fun writeExtraPayload(stream: ByteArrayStream, stringTable: StringTable, item: SerializedClassFields) {
        stream.writeInt(item.outerThisIndex)
