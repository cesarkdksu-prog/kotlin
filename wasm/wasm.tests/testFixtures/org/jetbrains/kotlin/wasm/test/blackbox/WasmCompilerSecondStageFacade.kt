...
class WasmCompilerSecondStageFacade private constructor(
    ...
    class Grouping(
        ...
        override fun transform(inputArtifact: GroupingStageInputArtifact): BinaryArtifacts.Wasm {
            val facade = WasmCompilerSecondStageFacade(testServices, customWebCompilerSettings)
    ...
    fun runCli(
        ...
        if (customLanguageFeatures.any {
                val feature = LanguageFeature.fromString(it.removePrefix("+").removePrefix("-")) ?: error("Unknown language feature '$it'")
                feature.testOnly
            }
        ) { // example: to enable `+ImplicitSignedToUnsignedIntegerConversion` via CLI in tests like `signedToUnsignedConversions.kt`
