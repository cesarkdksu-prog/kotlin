 ...
 */
abstract class AbstractThisLabelResolveTest : AbstractFirLazyDeclarationResolveTestCase() {
    override fun doTestByMainModuleAndOptionalMainFile(mainFile: KtFile?, mainModule: KtTestModule, testServices: TestServices) {
        ...
        val declaration = when (file) {
            is KtFile -> testServices.expressionMarkerProvider.getBottommostElementOfTypeAtCaret<KtFunction>(file)
        ...
        val symbolSuppliers = when (declaration) {
            ...
            is KtFunctionLiteral -> listOf(
                { s: KaSymbol -> s } to "function literal",
            )
    ...
    private fun KaSession.testSymbol(
        ...
        val deprecationStatus = buildString {
            (targetSymbol as? KaFunctionSymbol)?.thisLabels?.forEach {
                appendLine("this@label: $it")
            }
        }
...
abstract class AbstractSourceThisLabelResolveTest : AbstractThisLabelResolveTest() {
